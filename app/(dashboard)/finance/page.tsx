      ...
      <div className="h-64 px-5">
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={chartData} margin={{ top: 10, right: 0, left: 0, bottom: 0 }}>
            ...
            />
            <Bar dataKey="revenue" fill={colors.revenue} radius={[4, 4, 0, 0]} maxBarSize={32} stackId="a" />
            <Bar dataKey="projected" fill={colors.revenue} radius={[4, 4, 0, 0]} maxBarSize={32} stackId="a" fillOpacity={0.35} />
            <Bar dataKey="expenses" fill={colors.expenses} radius={[0, 0, 4, 4]} maxBarSize={32} stackId="a" />
            <Bar dataKey="projectedExpenses" fill={colors.expenses} radius={[0, 0, 4, 4]} maxBarSize={32} stackId="a" fillOpacity={0.35} />
            {(() => {
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button
          type="button"
          className="flex items-center gap-2 rounded px-1.5 py-0.5 -mx-1.5 -my-0.5 text-sm outline-none hover:bg-foreground/[0.04] transition-colors text-left w-full"
        >
          {selected ? (
            <>
            ...
            <span className="text-foreground/30">Select project...</span>
          )}
        </button>
      </DropdownMenuTrigger>
...
function SortablePipelineGroup({ firstItem, children }: { firstItem: PipelineItem; children: React.ReactNode }) {
  ...
  return (
    <div ref={setNodeRef} style={style} className="group/row flex items-center gap-2 px-5 py-2.5 border-b border-foreground/[0.06]">
      <button
