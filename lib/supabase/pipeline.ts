...
export async function createPipelineItem(
  projectId: string,
  item: { client_id: string; amount: number; expected_date: string; label?: string; sort_order: number }
...
export async function updatePipelineItem(
  id: string,
  updates: Partial<Pick<PipelineItem, 'client_id' | 'amount' | 'expected_date' | 'label' | 'sort_order'>>
): Promise<void> {
  const supabase = createClient()
